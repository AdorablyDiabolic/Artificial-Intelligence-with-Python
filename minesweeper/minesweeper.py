...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        if undetermined_cells:
            new_sentence = Sentence(set(undetermined_cells), count - count_mines)
    ...
    def update_knowledge_base(self):
        ...
        # After updating knowledge with subsets, recheck for any new mines or safes
        self.infer_new_safes_and_mines()

    def infer_new_safes_and_mines(self):
        changed = True
        while changed:
            changed = False
            all_safe = set()
            ...
            for sentence in self.knowledge:
                if len(sentence.cells) == sentence.count:
                    all_mines.update(sentence.cells)
                if sentence.count == 0:
                    all_safe.update(sentence.cells)

            # Mark all detected mines and safes
