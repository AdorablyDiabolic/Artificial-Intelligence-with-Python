...
def player(board):
    ...
    for row in range(len(board)):  # nested loop checking for x or o in row and colum
...
def actions(board):
    ...
    for row in range(len(board)):
        for col in range(len(board[0])):
            if board[row][col] == EMPTY:  # check if empty spotes
...
def result(board, action):
    """
    Returns the board that results from making move (i = row, j = col ) on the board.
    """
